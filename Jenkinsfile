...
pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
        git branch: 'master', credentialsId: 'GithubCred', url: 'https://github.com/pc9733/springboot-build-pipeline.git'
      }
    }
   ...
   stage('Stage I: Build') {
      steps {
        ...
        sh "export JAVA_HOME=/usr/lib/jvm/java-1.8.0-amazon-corretto.x86_64/jre/bin/java; mvn clean package "
      }
    }
   ...
   stage('Stage VII: Scan Image ') {
      steps { 
        ...
        sh "trivy image --scanners vuln --offline-scan piyush9734/democicd:latest > trivyresults.txt"
        }
    }
   ...
   stage('Stage VIII: Smoke Test ') {
      steps { 
        ...
        sh "docker run -d --name smokerun -p 8080:8080 piyush9734/democicd"
